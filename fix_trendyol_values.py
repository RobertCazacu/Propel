"""
Fix Trendyol values.parquet
Problema: Result_5.xlsx nu are coloana category_id.
Solutia: join cu characteristics.parquet pe characteristic_id -> category_id.

Rulare: python fix_trendyol_values.py
"""
import pandas as pd
from pathlib import Path
from openpyxl import load_workbook

VALUES_XLSX  = Path("C:/Users/manue/Desktop/Result_5.xlsx")
PARQUET_DIR  = Path("data/Trendyol")
...
def main():
    ...
    print(f"Deschid {VALUES_XLSX} in mod streaming...")
    wb = load_workbook(VALUES_XLSX, read_only=True, data_only=True)
    ws = wb.active

    rows_iter = ws.iter_rows(values_only=True)
    header = next(rows_iter)
    print(f"  Coloane gasite in fisier: {list(header)}")
    ...
    idx_char_id = col_idx("characteristic_id", "char_id", "emag_characteristic_id")
    idx_value   = col_idx("value", "value_ro", "valoare", "val")
    idx_name    = col_idx("name", "characteristic_name", "char_name")

    if idx_char_id is None or idx_value is None:
        print("EROARE: nu am gasit coloanele necesare!")
        print(f"  characteristic_id index: {idx_char_id}")
    ...
    if idx_name is not None:
        print(f"  Coloana name:     index {idx_name} ('{header[idx_name]}')")
    ...
    for row in rows_iter:
        ...
        if idx_name is not None and row[idx_name] is not None:
            entry["_name"] = str(row[idx_name]).strip()
        ...
        if len(batch) >= BATCH_SIZE:
            ...
            # Prefer name from file if available, fallback to chars join
            if "_name" in merged.columns:
                merged["characteristic_name"] = merged["_name"].where(
                    merged["_name"].notna() & (merged["_name"] != ""),
                    merged["characteristic_name"]
                )
